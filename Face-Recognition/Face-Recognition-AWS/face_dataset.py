...
while(True):
    ...
    faces = face_detector.detectMultiScale(gray, 1.3, 5)#detects the faces

    for (x,y,w,h) in faces:

        cv2.rectangle(img, (x,y), (x+w,y+h), (255,0,0), 2)    #draws the rectangle around the detected face
        count += 1
...
cam.release()
cv2.destroyAllWindows()#it will end the instance of cv2
